...
TABLE_DOCS: dict[str, str] = {
    ...
    "market_flow": "资金去向：每天一行，宽基/行业 ETF 申赎 + 成交额 + 杠杆 + 集中度（要有历史才能回测）",
...
FIELD_DOCS: dict[str, dict[str, tuple[str, str]]] = {
    "market_flow": {
        "trade_date": ("交易日", "一天一行"),
        "broad_etf_inflow": ("宽基 ETF 净流入", "单位：亿元；份额变化 × 价格，同一来源内比前后两天"),
        "sector_etf_inflow": ("行业/主题 ETF 净流入", "单位：亿元；与宽基反向时就是轮动的直接证据"),
        "etf_covered": ("参与统计的 ETF 只数", "有份额变化的；深市只有最新快照，补不出历史"),
        "amount": ("全市场个股成交额", "单位：万亿；只数个股，不含 ETF 与指数"),
        "amount_ratio": ("放量倍数", "成交额 ÷ 近 60 个交易日中位数"),
        "margin_delta": ("融资余额变化", "单位：亿元；按各市场自己的披露日对齐后相加"),
        "margin_balance": ("融资余额", "单位：万亿；融资余额 + 融券余量金额"),
        "top100_pct": ("成交额前 100 只占比", "单位：%；集中度读数，看分位不看绝对值"),
        "hhi": ("成交额 HHI", "Σ(份额²) × 10000，对头部特别敏感"),
        "big_count": ("成交额 ≥50 亿只数", "「大票活跃度」的计数口径"),
        "index_pct": ("当日沪深300涨跌幅", "单位：%；分档回测的落点"),
        "updated_at": ("最后更新时间", ""),
    },
    "support_days": {
